...
/* Expected schema — see schema.sql in the project root for the full,
   copy-pasteable version (tables, RLS, and the phone_login function that
   backs the WhatsApp OTP sign-in below, called only from the verify-otp
   Edge Function in supabase/functions/). Summary:

   app_users(id, phone, name, created_at)
   ...
   access at all) — every read/write to it goes through phone_login,
   which only the verify-otp Edge Function can call (service-role key). */
...
en: {
  ...
  signInTitle:"Sign in with WhatsApp",
  signInHint:"Browsing, liking, and saving never need an account — this is only for posting or contacting someone.",
  lblPhone:"WhatsApp number", sendCodeBtn:"Send code on WhatsApp",
  otpSentPrefix:"Code sent to ", lblOtpCode:"6-digit code", verifyBtn:"Verify & sign in",
  resendCodeBtn:"Resend code", resendCodeWait:"Resend in {s}s",
  otpErrorInvalidPhone:"Enter a valid WhatsApp number, starting with +.",
  otpErrorSendFailed:"Couldn't send the code — try again.",
  otpErrorWrongCode:"That code is wrong or expired.",
  otpErrorOffline:"Sign-in needs the live server — it isn't set up in this demo yet.",
  signInBtnShort:"Sign in with WhatsApp",
},
ar: {
  ...
  signInTitle:"سجّل الدخول عبر واتساب",
  signInHint:"التصفح والإعجاب والحفظ لا يحتاج حساب أبداً — الحساب فقط للنشر أو التواصل مع أحد.",
  lblPhone:"رقم واتساب", sendCodeBtn:"أرسل الرمز عبر واتساب",
  otpSentPrefix:"تم إرسال الرمز إلى ", lblOtpCode:"الرمز المكوّن من ٦ أرقام", verifyBtn:"تحقق وسجّل الدخول",
  resendCodeBtn:"أعد إرسال الرمز", resendCodeWait:"إعادة الإرسال خلال {s} ثانية",
  otpErrorInvalidPhone:"أدخل رقم واتساب صحيح يبدأ بـ +.",
  otpErrorSendFailed:"تعذر إرسال الرمز — حاول مرة أخرى.",
  otpErrorWrongCode:"الرمز غير صحيح أو منتهي الصلاحية.",
  otpErrorOffline:"تسجيل الدخول يحتاج الخادم الفعلي — غير مُفعّل في هذا العرض التجريبي.",
  signInBtnShort:"سجّل الدخول عبر واتساب",
},
ur: {
  ...
  signInTitle:"واٹس ایپ سے سائن ان کریں",
  signInHint:"براؤزنگ، پسند کرنا اور محفوظ کرنا کبھی اکاؤنٹ کا تقاضا نہیں کرتے — یہ صرف پوسٹ کرنے یا رابطہ کرنے کے لیے ہے۔",
  lblPhone:"واٹس ایپ نمبر", sendCodeBtn:"واٹس ایپ پر کوڈ بھیجیں",
  otpSentPrefix:"کوڈ بھیج دیا گیا: ", lblOtpCode:"6 ہندسوں کا کوڈ", verifyBtn:"تصدیق کریں اور سائن ان کریں",
  resendCodeBtn:"کوڈ دوبارہ بھیجیں", resendCodeWait:"{s} سیکنڈ میں دوبارہ بھیجیں",
  otpErrorInvalidPhone:"+ سے شروع ہونے والا درست واٹس ایپ نمبر درج کریں۔",
  otpErrorSendFailed:"کوڈ بھیجا نہیں جا سکا — دوبارہ کوشش کریں۔",
  otpErrorWrongCode:"یہ کوڈ غلط ہے یا میعاد ختم ہو چکی ہے۔",
  otpErrorOffline:"سائن ان کے لیے لائیو سرور درکار ہے — یہ ڈیمو میں سیٹ اپ نہیں ہے۔",
  signInBtnShort:"واٹس ایپ سے سائن ان کریں",
}
};
...
/* ============================= Sign-in (WhatsApp OTP via Twilio Verify) ============================= */
const AUTH_STORAGE_KEY = 'sanad_user';
let otpPhone = null; // the phone number a code was just sent to
...
function resetOtpModal(){
  document.getElementById('otpPhoneStep').style.display = 'block';
  ...
  document.getElementById('btnSendOtp').textContent = t('sendCodeBtn');
  otpPhone = null;
...
function startResendCooldown(seconds){
  ...
  btn.disabled = true;
  btn.textContent = fmt('resendCodeWait', { s: remaining });
...
async function sendOtp(){
  ...
  if(!/^\+[1-9]\d{7,14}$/.test(phone)){ showAuthError(t('otpErrorInvalidPhone')); return; }

  if(!SUPABASE_CONFIGURED){
    showAuthError(t('otpErrorOffline'));
    return;
  }
  ...
  btn.disabled = true;
  try{
    const { data, error } = await supabaseClient.functions.invoke('send-otp', { body: { phone } });
    if(error || (data && data.error)){
      console.error('Sanad: send-otp failed:', error || data.error);
      showAuthError(t('otpErrorSendFailed'));
      btn.disabled = false;
      return;
    }
    otpPhone = phone;
    document.getElementById('otpPhoneStep').style.display = 'none';
    document.getElementById('otpCodeStep').style.display = 'block';
    document.getElementById('otpSentHint').textContent = t('otpSentPrefix') + phone;
    startResendCooldown(30);
  } catch(err){
    console.error('Sanad: could not reach send-otp:', err);
    showAuthError(t('otpErrorSendFailed'));
    btn.disabled = false;
  }
}
...
async function verifyOtp(){
  ...
  if(!otpPhone || !/^\d{4,10}$/.test(code)){ showAuthError(t('otpErrorWrongCode')); return; }

  const btn = document.getElementById('btnVerifyOtp');
  btn.disabled = true;
  try{
    const { data, error } = await supabaseClient.functions.invoke('verify-otp', { body: { phone: otpPhone, code, name: name || undefined } });
    if(error || !data || data.error || !data.user){
      console.error('Sanad: verify-otp failed:', error || (data && data.error));
      showAuthError(t('otpErrorWrongCode'));
      btn.disabled = false;
      return;
    }
    const row = data.user;
    appUser = { id: row.id, name: row.name, phone: row.phone, createdAt: row.created_at, sessionToken: row.session_token };
  } catch(err){
    console.error('Sanad: could not reach verify-otp:', err);
    showAuthError(t('otpErrorSendFailed'));
    btn.disabled = false;
    return;
  }
...
document.getElementById('btnResendOtp').addEventListener('click', ()=>{
  document.getElementById('si-phone').value = otpPhone || '';
  document.getElementById('otpPhoneStep').style.display = 'block';
  document.getElementById('otpCodeStep').style.display = 'none';
  sendOtp();
});
